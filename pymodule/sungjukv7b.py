...
def saveSungJuk(name):
    sj = [name, sungjuks[name][0], sungjuks[name][1], sungjuks[name][2],
         sungjuks[name][3], sungjuks[name][4], sungjuks[name][5]]

    with open('./sungjukv7b.csv', 'a', encoding='utf-8', newline='') as f:
        wtr = csv.writer(f)
        wtr.writerow(sj)

def loadSungJuk():
    with open('./sungjukv7b.csv', 'r', encoding='utf-8', newline='') as f:
        rdr = csv.reader(f)
        for sj in rdr:
            sungjuks[sj[0]] = [int(sj[1]), int(sj[2]), int(sj[3]), int(sj[4]), float(sj[5]), sj[6]]
